...
void loop() {
  if (awaitingInput && Serial.available()) {
    ...
    if (indexToRemove >= 0 && indexToRemove < 9) { // Ensure valid index and space for adjacent index
      ...
      for (int i = indexToRemove; i <       values[8] = rightValue; // Move right value to the end of the array
    ...
    the right value to the end of the array. The updated array is then printed to the serial monitor.
```